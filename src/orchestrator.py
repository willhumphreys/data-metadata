...
def clean_directory(directory):
    ...
    if os.path.exists(directory):
        ...
        if directory and directory != '/' and os.path.isdir(directory):
            ...
            safe_to_remove = not directory.startswith(
...
def numpy_encoder(obj):
    ...
    """
    if isinstance(obj,
                  (np.integer, np.int_, np.intc, np.intp, np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16,
                   np.uint32, np.uint64)):
        return int(obj)
...
def create_batch_parameters(group_tag: str, scenario: str, ticker: str, trade_type: str, s3_key_min: str,
                            atr: bool) -> BatchParameters:
    """
...
def format_scenario_string(s_min, s_max, s_step, l_min, l_max, l_step, o_min, o_max, o_step, d_min, d_max, d_step,
                           out_min, out_max, out_step):
...
def generate_and_submit_scenarios(time_to_place_range: int, time_to_hold_range: int,  # Keep this argument
    ...
    print(
        f"Calculated Initial Stop/Limit Step Size (hold_range / {INITIAL_STEP_SIZE_HOLD_RANGE_DENOMINATOR}): {calculated_initial_stop_limit_step_size}")
    ...
    while True:
        ...
        print(
            f"Minimum acceptable stop/limit size ({MIN_STOP_LIMIT_PERCENTAGE * 100}% of range): {min_acceptable_size}")
        ...
        initial_scenario_params = {'s_min': stop_min, 's_max': stop_max, 's_step': stop_step, 'l_min': limit_min,
            'l_max': limit_max, 'l_step': limit_step, 'o_min': offset_min, 'o_max': offset_max, 'o_step': offset_step,
            'd_min': duration_min, 'd_max': duration_max, 'd_step': duration_step, 'out_min': output_min,
            'out_max': output_max, 'out_step': output_step, }
        ...
        while scenarios_to_process:
            ...
            if combinations == float('inf'):
                print(
                    f"  -> Skipping scenario chunk due to overflow/excessive size: {format_scenario_string(**current_params)}")
                continue
            ...
            if combinations <= MAX_COMBINATIONS_PER_JOB:
                ...
                num_s = (current_params['s_max'] - current_params['s_min']) // current_params['s_step'] if \
                current_params['s_step'] > 0 and current_params['s_max'] >= current_params['s_min'] else 0
                num_l = (current_params['l_max'] - current_params['l_min']) // current_params['l_step'] if \
                current_params['l_step'] > 0 and current_params['l_max'] >= current_params['l_min'] else 0
                num_o = (current_params['o_max'] - current_params['o_min']) // current_params['o_step'] if \
                current_params['o_step'] > 0 and current_params['o_max'] >= current_params['o_min'] else 0
                ...
                if num_o > 0 and num_o >= num_s and num_o >= num_l:
                    ...
                    print(
                        f"  -> WARNING: Cannot determine parameter to split for large scenario ({combinations} combinations). Params: {current_params}. Keeping as is. This WILL exceed limits.")
                    scenario_str = format_scenario_string(**current_params)
                    final_scenario_strings.append(
                        {"scenario": scenario_str, "combinations": combinations, "params": current_params})
                ...
                if p_step <= 0:
                    print(
                        f"  -> WARNING: Step size for parameter '{split_param}' is zero or negative ({p_step}). Cannot split. Keeping chunk as is.")
                    scenario_str = format_scenario_string(**current_params)
                    final_scenario_strings.append(
                        {"scenario": scenario_str, "combinations": combinations, "params": current_params})
                    continue
                ...
                if num_steps_total < 1:  # Changed from mid_point_steps <= 0 to catch single-step ranges
                    print(
                        f"  -> WARNING: Cannot split {split_param} range ({p_min}..{p_max} step {p_step}) further (only {num_steps_total + 1} value(s)). Keeping as is. This might exceed limits.")
                    scenario_str = format_scenario_string(**current_params)
                    final_scenario_strings.append(
                        {"scenario": scenario_str, "combinations": combinations, "params": current_params})
                    continue
                ...
                params2[
                    f'{split_param}_min'] = p_split_val + p_step  # Second half starts one step after the middle point
                ...
                if params2[f'{split_param}_max'] >= params2[f'{split_param}_min']:
                    scenarios_to_process.insert(0,
                                                params2)  # else: # Optional logging for discarded splits  #      print(f"  -> Discarding invalid split range 2 for {split_param}: min={params2[f'{split_param}_min']}, max={params2[f'{split_param}_max']}")

        # --- End of Inner Splitting Loop ---

        num_scenario_pairs = len(final_scenario_strings)
        print(
            f"Generated {num_scenario_pairs} scenario pairs with Stop/Limit step {current_stop_limit_step_size}, Offset step {current_offset_step_size}.")

        if num_scenario_pairs <= MAX_SCENARIO_PAIRS:
            print(
                f"Number of scenario pairs ({num_scenario_pairs}) is within the limit ({MAX_SCENARIO_PAIRS}). Proceeding to submission.")
        ...
        stop_min_safe = -time_to_hold_range - 1;
        stop_max_safe = -1
        limit_min_safe = 1;
        limit_max_safe = time_to_hold_range + 1
        ...
        if total_sl_range_magnitude > 0 and current_stop_limit_step_size > total_sl_range_magnitude:
            print(
                f"ERROR: Stop/Limit step size ({current_stop_limit_step_size}) has become larger than the combined Stop/Limit range magnitude ({total_sl_range_magnitude}). Stopping generation.")
            final_scenario_strings = []  # Ensure no jobs are submitted
            # Return an error state or specific message
            return {'statusCode': 500, 'body': json.dumps({
                                                              'message': f'Failed to generate scenarios within limits. Stop/Limit step size ({current_stop_limit_step_size}) grew larger than the price range ({total_sl_range_magnitude}).'})}
        ...
        if current_stop_limit_step_size > abs(time_to_hold_range * 100):  # Arbitrary large multiplier
            print(
                f"ERROR: Stop/Limit step size ({current_stop_limit_step_size}) has grown excessively large. Stopping generation.")
            final_scenario_strings = []
            return {'statusCode': 500, 'body': json.dumps(
                {'message': 'Failed to generate scenarios within limits, step size grew excessively large.'})}
    ...
    for scenario_info in final_scenario_strings:
        ...
        # Submit LONG
        try:
            batch_params_long = create_batch_parameters(group_tag=group_tag, scenario=scenario_str, ticker=ticker,
                                                        trade_type="long", s3_key_min=s3_key_min, atr=False)
            if batch_client:
        ...
        try:
            batch_params_long = create_batch_parameters(group_tag=group_tag, scenario=scenario_str, ticker=ticker,
                                                        trade_type="long", s3_key_min=s3_key_min, atr=True)
            if batch_client:
                put_trade_job_on_queue(batch_params_long, batch_client)
            else:
                put_trade_job_on_queue(batch_params_long, None)  # Call mock version
        ...
        # # Submit SHORT  # try:  #     batch_params_short = create_batch_parameters(group_tag, scenario_str, ticker, "short", s3_key_min)  #     if batch_client:  #         put_trade_job_on_queue(batch_params_short, batch_client)  #     else:  #         put_trade_job_on_queue(batch_params_short, None) # Call mock version  #     submitted_job_count += 1  #     total_combinations_submitted += combinations  #     print(f"  Submitted SHORT job: {batch_params_short.trades_job_name} ({combinations} combinations)")  # except Exception as e:  #     print(f"  ERROR Submitting SHORT job for scenario {scenario_str}: {e}")
    ...
    output_summary = {'ticker': ticker, 'group_tag': group_tag, 's3_key_min': s3_key_min,
        ...
        'final_stop_limit_step_size': current_stop_limit_step_size, 'final_offset_step_size': final_offset_step_size,
        # Report final offset step
        'max_combinations_per_job': MAX_COMBINATIONS_PER_JOB, 'max_scenario_pairs_limit': MAX_SCENARIO_PAIRS,
        'price_analysis': {'offset_window_hours': TIME_WINDOW_OFFSET_HOURS, 'offset_max_range': time_to_place_range,
            'stop_limit_window_hours': TIME_WINDOW_STOP_LIMIT_HOURS, 'stop_limit_max_range': time_to_hold_range, },
        'submitted_job_pairs': submitted_job_count // 2, 'total_combinations_submitted': total_combinations_submitted,
...
def pipeline(ticker=None, output_dir=DEFAULT_OUTPUT_DIR, clean_output=True, group_tag=None, s3_key_min=None,
    ...
    if s3_key_min is None: print("Error: S3 key for minute data (s3_key_min) is required"); raise ValueError(
        "S3 key for minute data is required")
    if group_tag is None: print("Error: Group tag is required for job identification"); raise ValueError(
    ...
    if not bucket: print("Error: Environment variable MOCHI_DATA_BUCKET is not set."); raise ValueError(
        "MOCHI_DATA_BUCKET environment variable not set.")

    file_path = download_from_s3(s3_key_min, output_dir, bucket=bucket)
    if not file_path or not os.path.exists(file_path): print(
        f"Error: Failed to download or locate data file from {s3_key_min}"); raise ValueError(
        f"Failed to download data from {s3_key_min}")
    ...
    if time_to_hold_range <= 0:
        print(
            f"Warning: Calculated time_to_hold_range is {time_to_hold_range}. Check price data and window. Proceeding might lead to issues.")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Download stock data, calculate price ranges, generate and submit backtesting scenarios.')
    parser.add_argument('--ticker', type=str, required=True, help='Stock ticker symbol (e.g., AAPL)')
    parser.add_argument('--s3-key-min', type=str, required=True,
                        help='S3 key for the minute data CSV file (e.g., path/to/TICKER-1mF.csv)')
    parser.add_argument('--group-tag', type=str, required=True,
                        help='A tag to group related AWS Batch jobs (e.g., weekly-run-2025-04-20)')
    parser.add_argument('--output-dir', type=str, default=DEFAULT_OUTPUT_DIR,
                        help=f'Local directory for downloads (default: {DEFAULT_OUTPUT_DIR})')
    ...
    parser.add_argument('--hold-days', type=int, default=DEFAULT_HOLD_DAYS,
                        help=f'Fixed trade holding duration in days (default: {DEFAULT_HOLD_DAYS})')
    parser.add_argument('--fill-hours', type=int, default=DEFAULT_FILL_HOURS,
                        help=f'Fixed time allowed for order fill in hours (default: {DEFAULT_FILL_HOURS})')
    ...
    if not re.match(r'^[a-zA-Z0-9_\-]+$', args.group_tag):
        print(
            "Warning: Group tag contains characters other than letters, numbers, underscore, hyphen. This might cause issues in some systems.")
...
if __name__ == "__main__":
    ...
    if 'AWS_DEFAULT_REGION' not in os.environ and 'AWS_REGION' not in os.environ:
        print(
            "WARNING: AWS region is not explicitly set via AWS_DEFAULT_REGION or AWS_REGION environment variables. Boto3 might use defaults or fail.")  # Optionally set a default mock region for testing  # os.environ['AWS_DEFAULT_REGION'] = 'us-east-1'  # print("Mocking AWS_DEFAULT_REGION='us-east-1'")
